...
// The number of columns of the first matrix is not equal to the number of rows
...
// Minor of A[k][l]
double S21Matrix::Minor(int k, int l) {
  double res = 0.0;
...
// Return (-1) ^ count of row switching
int S21Matrix::Gauss() {
  double coef;
  ...
  for (int i = 0; i < rows_; i++) {
    ...
    if (std::fabs(matrix_[iMax][i]) > S21Matrix::eps) {
      ...
      if (i != iMax) countSwap++;
      for (int k = i + 1; k < rows_; k++) {
        coef = matrix_[k][i] / matrix_[i][i];
        for (int j = i; j < cols_; j++) matrix_[k][j] -= coef * matrix_[i][j];
      }
    }
  }
